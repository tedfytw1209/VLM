...
for split, parquet_files in splits.items():
    ...
    for i, (img_bytes, img_path) in tqdm(
        enumerate(zip(image_bytes, image_paths)), total=len(image_bytes), desc=f"Processing {split} images"
    ):
    ...
    metadata = pd.DataFrame({"file_name": image_paths, "question": questions, "answer": answers})
